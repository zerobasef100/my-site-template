...
export function Hero() {
  ...
  // 초기 데이터
  const defaultSocialLinks = [{"name":"계정","icon":"instagram","url":"https://www.instagram.com/jn_woo000?igsh=MTgzMjFoZWJxZ2ow"},{"name":"클라이밍","icon":"instagram","url":"https://www.instagram.com/wu.climb?igsh=MWpqcDRhZTZmMG92YQ=="},{"name":"메일","icon":"mail","url":"swagcon12@naver.com"}]
  ...
  // 배경 기본 객체 (undefined 방지)
  const defaultBg = {
    image: defaultInfo.backgroundImage || "",
    video: defaultInfo.backgroundVideo || "",
    color: "",
    opacity: defaultInfo.backgroundOpacity ?? 0.3,
  }

  const [backgroundData, setBackgroundData] = useState(defaultBg)
  ...
  useEffect(() => {
    const savedData = getData("hero-info") as Partial<typeof defaultInfo> | null
    if (savedData) {
      const mergedHero = { ...defaultInfo, ...savedData }
      setHeroInfo(mergedHero)
    }
  ...
  return (
    <EditableBackground
      ...
      onChange={(data) => {
        const newData = { ...defaultBg, ...backgroundData, ...data }
        setBackgroundData(newData)

        // 저장
        saveData("hero-background", newData)

        // heroInfo에도 반영(파일 저장용 필드와 동기화)
        const updatedHeroInfo = {
          ...heroInfo,
          backgroundImage: newData.image,
          backgroundVideo: newData.video,
          backgroundOpacity: newData.opacity,
        }
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
          <div className="bg-background border rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowSocialEditor(false)} className="p-1 hover:bg-muted rounded-lg">
                <X className="h-5 w-5" />
              </button>
            </div>
              ...
              {socialLinks.map((link, index) => {
                ...
                return (
                  <div key={index} className="flex items-center gap-3 p-3 border rounded-lg">
                    ...
                    {/* 삭제 버튼 */}
                    <button onClick={() => removeSocialLink(index)} className="p-2 text-destructive hover:bg-destructive/10 rounded-lg">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                )
            ...
            <div className="mt-6 pt-4 border-t">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowSocialEditor(false)} className="flex-1 py-2 bg-muted text-foreground rounded-lg hover:bg-muted/80">
                  닫기
                </button>
                <button
                  onClick={async () => {
